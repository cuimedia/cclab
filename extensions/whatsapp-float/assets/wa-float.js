(function () {
  ...
  function resolveConfigUrl(shop) {
    var candidates = [];

    if (CURRENT_SCRIPT) {
      var attrEndpoint = CURRENT_SCRIPT.getAttribute('data-wa-float-endpoint');
      if (!attrEndpoint && CURRENT_SCRIPT.dataset) {
        attrEndpoint = CURRENT_SCRIPT.dataset.waFloatEndpoint;
      }
      if (attrEndpoint) {
        candidates.push({ url: attrEndpoint, type: 'endpoint' });
      }
      var attrBase = CURRENT_SCRIPT.getAttribute('data-wa-float-base');
      if (!attrBase && CURRENT_SCRIPT.dataset) {
        attrBase = CURRENT_SCRIPT.dataset.waFloatBase;
      }
      if (attrBase) {
        candidates.push({ url: attrBase, type: 'base' });
      }
    }

    if (typeof window !== 'undefined') {
      if (window.__WA_FLOAT_ENDPOINT__) {
        candidates.push({ url: window.__WA_FLOAT_ENDPOINT__, type: 'endpoint' });
      }
      if (window.__WA_FLOAT_BASE_URL__) {
        candidates.push({ url: window.__WA_FLOAT_BASE_URL__, type: 'base' });
      }
      if (window.__WA_FLOAT_APP_URL__) {
        candidates.push({ url: window.__WA_FLOAT_APP_URL__, type: 'base' });
      }
    }

    for (var i = 0; i < candidates.length; i += 1) {
      var entry = candidates[i];
      var raw = String(entry && entry.url != null ? entry.url : '').trim();
      if (!raw) continue;

      try {
        var candidateUrl = new URL(raw, window.location.href);
        var path = candidateUrl.pathname || '/';

        if (entry.type === 'base') {
          if (path.endsWith('/')) {
            path = path.slice(0, -1);
          }
          if (!/\/public\/wa-float-config\/?$/.test(path)) {
            path = (path || '') + '/public/wa-float-config';
          }
        } else if (!/\/public\/wa-float-config\/?$/.test(path)) {
          if (path.endsWith('/')) {
            path = path.slice(0, -1);
          }
          path = (path || '') + '/public/wa-float-config';
        ...
        candidateUrl.pathname = path || '/public/wa-float-config';
        candidateUrl.searchParams.set('shop', shop);
  ...
  function fetchConfig(shop) {
    var endpoint = resolveConfigUrl(shop);
    if (!endpoint) {
      console.warn('[WhatsApp Float] Missing config endpoint. Provide data-wa-float-base or set window.__WA_FLOAT_BASE_URL__.');
      return;
    }
    ...
    fetch(endpoint, { credentials: 'omit' })
      ...
      .catch(function (err) {
        console.error('[WhatsApp Float] Failed to fetch config', err);
      });
  }
